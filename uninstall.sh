...
stop_services() {
    ...
    # Stop Grafana/Loki services (NOT user services!)
    for service in grafana loki promtail goaccess-dashboard; do
...
remove_files() {
    ...
    for service in api-gateway-watch grafana loki promtail goaccess-dashboard; do
    ...
    # Remove Grafana/Loki/Promtail directories
    for dir in /opt/grafana /opt/grafana-install /opt/loki /opt/promtail /var/www/dashboard /etc/goaccess; do
        if [ -d "$dir" ]; then
            rm -rf "$dir"
    ...
    # Remove Grafana/Loki scripts
    for script in goaccess-dashboard; do
